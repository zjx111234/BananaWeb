...
from clbbs.templatetags import custom
...
def articles_generate_html(re, obj_list):
    ...
    for obj in articles['data']:
        ...
        img_a = "<a href='/clbbs/article_detail/"+str(obj.id)+" '>"
        ele = "<div article_id='%d' class='article-box'>" % obj.id + \
              ...
              "<div class='article-brief col-md-8'><a class='article-title' href='/clbbs/article_detail/"+str(obj.id)+" '>" + "%s</a>" %obj.title + \
              "<div class ='article-brief-info' ><span class='author-name'>%s</span > " %obj.author.name +\
